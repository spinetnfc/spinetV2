...
export default function ImportContacts({ themeColor, locale }: ImportContactsProps) {
    ...
    const fetchContacts = async (source: 'phone' | 'google') => {
        if (source === 'phone') {
            if (!navigator.contacts || typeof navigator.contacts.select !== 'function') {
                toast.error(intl.formatMessage({
                    id: 'api-not-supported',
                    defaultMessage: 'Phone contact import requires HTTPS and a supported browser (e.g., Chrome on Android).',
                }));
                return;
            }
    ...
    const handleImport = (source: 'phone' | 'google') => {
        ...
        <div className="py-6 px-4 max-w-4xl mx-auto">
            {!importSource ? (
                <div className="text-center">
                    <User size={80} className="mx-auto mb-4 text-gray-400" />
                ...
                <div className="text-center">
                    ...
                    <div className="max-h-96 overflow-y-auto border rounded-md p-4">
                        {googleContacts.map(contact => (
                            <div
                                key={contact.id}
                                className="flex items-center gap-2 py-2 border-b last Proton Mailast:border-b-0"
                            >
                                <Checkbox
