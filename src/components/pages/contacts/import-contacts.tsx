...
export default function ImportContacts({ locale }: ImportContactsProps) {
  ...
  const handleFileUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.endsWith(".vcf") && file.type !== "text/vcard") {
      toast.error(
        intl.formatMessage({ id: "invalid-file", defaultMessage: "Please upload a valid vCard (.vcf) file." }),
      )
      return
    }
    ...
    try {
      ...
      if (parsedContacts.length === 0) {
        toast.error(
          intl.formatMessage({ id: "no-contacts-found", defaultMessage: "No valid contacts found in the file." }),
        )
  ...
  const handleConfirmGoogleSelection = async () => {
    ...
    setIsAdding(true)
    setAddProgress(0)
    const selectedContacts = googleContacts.filter((contact) => selectedGoogleContacts.includes(contact.id))
      ...
      if (!navigator.contacts || typeof navigator.contacts.select !== "function") {
        toast.error(
          intl.formatMessage({
            ...
            defaultMessage:
              "Phone contact import requires HTTPS and a supported browser (e.g., Chrome on Android). Try importing a vCard file.",
          }),
        )
        return
      }
        ...
        <div className="text-center">
          ...
          <div className="max-h-96 overflow-y-auto border rounded-md p-4">
            {googleContacts.map((contact) => (
              <div key={contact.id} className="flex items-center gap-2 py-2 border-b last:border-b-0">
                <Checkbox
          ...
          <div className="flex justify-center gap-4 mt-4">
            <Button onClick={handleConfirmGoogleSelection} className="flex items-center gap-2 bg-azure">
              <Check size={20} />
